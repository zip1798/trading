...
export class Backpack extends BaseExchange {
    constructor(
        apiKey: string,
        apiSecret: string,
        baseUrl: string = 'https://api.backpack.exchange/v1'
    ) {
    ...
    private getHeaders(path: string = '', params: any = {}): Record<string, string> {
        const timestamp = Date.now().toString();
        return {
            'BP-API-Key': this.apiKey,
            'BP-Timestamp': timestamp,
            'BP-Signature': this.signRequest(path, params),
